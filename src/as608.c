...
void    as608_add_finger( AS608* this )
{
    ...
    while( true ) {
        ...
        case 0:
            ...
            if( ensure == 0x00 ) {
                scib_msg( "scib: 得到一个指纹图像，生成特征中....\n\r" );
                ensure  =   this->gen_char(this, CHAR_BUFFER_1);
                if( ensure ==   0x00 ) {
                    scib_msg( "scib: 指纹图像获取成功....\n\r" );
                    process_num = 1;
                }else {
                    scib_msg( "scib: 指纹图像获取失败....\n\r" );
                    this->ensure_message( this, ensure );
                }
            }else {
                this->ensure_message(this, ensure);
        ...
        case 1:
            scib_msg( "scib: 请再次按下指纹.....\n\r" );
            ensure = this->get_image(this);
            if( ensure == 0x00 ) {
                ensure  =   this->gen_char(this, CHAR_BUFFER_2);
                if( ensure ==   0x00 ) {
                    scib_msg( "scib: 输入指纹成功....\n\r" );
                    process_num = 2;
                }else {
                    this->ensure_message( this, ensure );
                }
        ...
        case 2:
            scib_msg( "scib: 正在比对采集指纹是否正确 ... \n\r" );
            ensure  =   this->match(this);
            if( ensure == 0x00 ) {
                scib_msg( "scib: 指纹比对成功。。。!\n\r" );
                process_num = 3;
            }else {
                scib_msg( "scib: 指纹比对失败...\n\r" );
                process_num = 0;
            }
            DELAY_MS(800);
        break;
    ...
    if( ensure == 0x00 ) {
        ...
        if( ensure == 0x00 ) {
            ...
            if( ensure == 0x00 ) {
                ...
                error_count ++;
                if( error_count > 3 ) {
                    scib_msg( "scib:验证失败超过3次蜂鸣器噪起来：...\n\r" );
                    alarm_voice(500);
                    error_count = 0;
                }else {
                    scib_msg( "scib:校验失败，请重新按指纹，原因 如下：...\n\r" );
                }
                this->ensure_message( this, ensure );
            }
        }else {
...
uint8*    as608_ensure_message( AS608 *this ,uint8 ensure) {
    ...
    switch(ensure)
    {
        ...
        case  0x03:
            p="scib:录入指纹图像失败\r\n";break;
        case  0x04:
            p="scib:指纹图像太干、太淡而生不成特征\r\n";break;
        case  0x05:
            p="scib:指纹图像太湿、太糊而生不成特征\r\n";break;
        case  0x06:
            p="scib:指纹图像太乱而生不成特征\r\n";break;
        case  0x07:
            p="scib:指纹图像正常，但特征点太少（或面积太小）而生不成特征\r\n";break;
        case  0x08:
            p="scib:指纹不匹配\r\n";break;
        case  0x09:
            p="scib:没搜索到指纹\r\n";break;
        case  0x0a:
            p="scib:特征合并失败\r\n";break;
        case  0x0b:
            p="scib:访问指纹库时地址序号超出指纹库范围\r\n";
        case  0x10:
            p="scib:删除模板失败\r\n";break;
        case  0x11:
            p="scib:清空指纹库失败\r\n";break;
        case  0x15:
            p="scib:缓冲区内没有有效原始图而生不成图像\r\n";break;
        case  0x18:
            p="scib:读写 FLASH 出错\r\n";break;
        case  0x19:
            p="scib:未定义错误\r\n";break;
...
bool    as608_uart_process( struct as608_t *this, BYTE val ) {
    ...
    //scib_msg("scib: 设备响应完毕，接收到数据开始处理.... \n\r");
    this->recv_count = 0;
    return true;
}
